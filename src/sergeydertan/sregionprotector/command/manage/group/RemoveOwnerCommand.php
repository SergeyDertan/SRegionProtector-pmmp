...
final class RemoveOwnerCommand extends SRegionProtectorCommand
{
    private $regionManager;

    public function __construct(RegionManager $regionManager)
    {
        parent::__construct("rgremoveowner", "removeowner");
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void
    {
        ...
        if (!$region->isOwner($target)) {
            $this->messenger->sendMessage($sender, "command.{$this->msg}.not-a-owner", ["@region", "@target"], [$region->getName(), $target]);
            return;
        }
        $this->regionManager->removeMember($region, $target);
        $this->messenger->sendMessage($sender, "command.{$this->msg}.owner-removed", ["@region", "@target"], [$region->getName(), $target]);
    }
}
